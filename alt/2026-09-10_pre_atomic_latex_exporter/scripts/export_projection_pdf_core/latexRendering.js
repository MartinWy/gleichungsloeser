...
function normalizeSuperscriptLatex(latex = "") {
    return String(latex || "").replace(/\s*\\cdot\s*/g, "\\cdot{}");
}

function renderPowerExponentLatex(node, options = {}, shellColorName = null) {
    if (node?.exponentNode) {
        return wrapLatexColor(
            `{\\scriptstyle ${normalizeSuperscriptLatex(renderNodeToLatex(node.exponentNode, [], options))}}`,
            shellColorName
        );
    }
...
function cloneRenderNodeForProjectedPowerExponent(node, preferExplicitSeparator = false) {
    if (!node || typeof node !== "object") {
        return null;
    }
    ...
    switch (node.type) {
        case "sequence":
            return {
                ...node,
                items: (node.items || [])
                    .map((item) => cloneRenderNodeForProjectedPowerExponent(item, preferExplicitSeparator))
                    .filter(Boolean)
            };
        case "multiplication":
            return {
                ...node,
                left: cloneRenderNodeForProjectedPowerExponent(node.left, preferExplicitSeparator),
                right: cloneRenderNodeForProjectedPowerExponent(node.right, preferExplicitSeparator),
                separator: preferExplicitSeparator
                    ? (node.separator || "*")
                    : node.separator
            };
        case "group":
            return {
                ...node,
                content: cloneRenderNodeForProjectedPowerExponent(node.content, preferExplicitSeparator)
            };
        case "function":
            return {
                ...node,
                argument: cloneRenderNodeForProjectedPowerExponent(node.argument, preferExplicitSeparator),
                baseArgument: cloneRenderNodeForProjectedPowerExponent(node.baseArgument, preferExplicitSeparator)
            };
        case "root":
            return {
                ...node,
        ...
        case "power":
            return {
                ...node,
                base: cloneRenderNodeForProjectedPowerExponent(node.base, preferExplicitSeparator),
                exponentNode: cloneRenderNodeForProjectedPowerExponent(node.exponentNode, preferExplicitSeparator)
            };
        case "fraction":
            return {
                ...node,
                numerator: cloneRenderNodeForProjectedPowerExponent(node.numerator, preferExplicitSeparator),
                denominator: cloneRenderNodeForProjectedPowerExponent(node.denominator, preferExplicitSeparator)
            };
        case "negation":
            return {
                ...node,
...
function createFallbackTextRenderNode(cell) {
    if (!cell) {
        return null;
    }
    ...
    return {
        type: "text",
        text: cell.text || "",
        kind: cell.kind === "operator" ? "operator" : "text"
    };
}

function buildProjectedPowerExponentRenderNode(memberCells = []) {
    const exponentCells = sortRowCells(
        (memberCells || []).filter((cell) => cell?.projectionRole === "power_exponent")
    );

    if (exponentCells.length === 0) {
        return null;
    }

    const sourceText = exponentCells.map((cell) => cell?.text || "").join(" ");
    const preferExplicitSeparator = sourceText.includes("*");
    const items = exponentCells
        .map((cell) => (
            cloneRenderNodeForProjectedPowerExponent(cell?.renderNode, preferExplicitSeparator)
            || createFallbackTextRenderNode(cell)
        ))
        .filter(Boolean);
...
function buildVisiblePowerShellRenderNode(node = null, memberCells = [], options = {}) {
    if (!node || typeof node !== "object") {
        return null;
    }
    ...
    const renderNode = buildRenderNode(renderSource);
    const projectedExponentNode = buildProjectedPowerExponentRenderNode(memberCells);

    if (renderNode?.type === "power" && projectedExponentNode) {
        return {
            ...renderNode,
            exponentNode: projectedExponentNode
        };
    }
...
function buildFractionFormulaFromCells(cells, line, semanticLayout = null, displayLayout = null, stepIndex = 0, shellColorsEnabled = false, shellColorPolicy = null) {
    ...
    const hasInlineForeignProjection = (cells || []).some((cell) => (
        cell?.row === line.row
        && overlapsFractionSpan(cell)
    ));

    if (hasInlineForeignProjection) {
        return null;
    }
    ...
    const hasOnlyAdjacentRows = (cells || []).every((cell) => (
        !overlapsFractionSpan(cell)
        || cell.row === line.row - 1
        || cell.row === line.row + 1
    ));

    if (numerator.length === 0 || denominator.length === 0 || !hasOnlyAdjacentRows) {
        return null;
    }
...
function buildVisiblePowerShellSpecs(step, options = {}) {
    ...
    return [...groups.values()].map((group) => {
        ...
        if (isDirectPowerFullyNestedInsideVisibleShell) {
            return null;
        }
        ...
        return {
            ...
            renderNode: buildVisiblePowerShellRenderNode(group.node, memberCells, {
                ...options,
