...
const logFunction = Atomisierer.process("log(x)=1");
const logShell = logFunction.find((atom) => atom.type === "FUNCTION");
assert.ok(logShell);
assert.equal(logShell.name, "log");

const lnFunction = Atomisierer.process("ln(x)=1");
const lnShell = lnFunction.find((atom) => atom.type === "FUNCTION");
assert.ok(lnShell);
assert.equal(lnShell.name, "ln");

const lgFunction = Atomisierer.process("lg(x)=1");
const lgShell = lgFunction.find((atom) => atom.type === "FUNCTION");
assert.ok(lgShell);
assert.equal(lgShell.name, "lg");
...
const symbolicPower = Atomisierer.process("B^x=1");
assert.equal(symbolicPower[0].type, "POWER");
assert.equal(symbolicPower[0].content[0].value, "B");
assert.equal(symbolicPower[0].exponent, "x");
assert.equal(symbolicPower[0].exponentNodes[0].value, "x");
assert.equal(symbolicPower[1].value, "=");
assert.equal(symbolicPower[2].value, "1");
