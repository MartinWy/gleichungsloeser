...
function collectFutureAnchorSideReserves(theoryRows = []) {
    ...
    for (let rowIndex = theoryRows.length - 1; rowIndex >= 0; rowIndex -= 1) {
        ...
        atoms.forEach((atom, index) => {
            ...
            if (index < anchorIndex && trailing > 0) {
                collectDeepBoundaryIdsFromCollection(sourceCollection, "right").forEach((id) => {
                    leftFutureReserves.set(id, Math.max(leftFutureReserves.get(id) || 0, trailing));
                });
            }

            if (index > anchorIndex && leading > 0 && !preservesEquationFacingEdge(atom, "right")) {
                collectDeepBoundaryIdsFromCollection(sourceCollection, "left").forEach((id) => {
                    rightFutureReserves.set(id, Math.max(rightFutureReserves.get(id) || 0, leading));
                });
            }
        });
    }
...
function computePackedStartCols(atoms, previousFootprint, futureReserves = null) {
    ...
    for (let index = anchorIndex - 1; index >= 0; index -= 1) {
        ...
        const reserveShift = resolveFutureAnchorSideReserve(atoms[index], "left", futureReserves);
    ...
    for (let index = anchorIndex + 1; index < atoms.length; index += 1) {
        ...
        const reserveShift = resolveFutureAnchorSideReserve(atoms[index], "right", futureReserves);
